...
# Placeholder TaskManager class to match ingestion_service.py usage
class TaskManager:
    """Manages concurrent execution of a batch of coroutines."""

    def __init__(self, max_concurrent: int = 10) -> None:
        self.semaphore = asyncio.Semaphore(max_concurrent)

    async def add_batch_coroutines(
        self, coroutines: list[Coroutine[Any, Any, T]]
    ) -> list[T]:
        """Runs a list of coroutines concurrently, limiting concurrency with a semaphore."""
        tasks = []
        results_with_exceptions: list[T | BaseException] = []

        async def _wrapper(coro: Coroutine[Any, Any, T]) -> T:
            async with self.semaphore:
                return await coro

        for coro_item in coroutines:
