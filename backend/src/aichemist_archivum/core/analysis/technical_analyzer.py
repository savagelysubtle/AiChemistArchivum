...
# Optional domain imports - only needed for TechnicalCodeAnalyzer class
try:
    from aichemist_archivum.domain.entities.code_artifact import CodeArtifact
    from aichemist_archivum.domain.repositories.code_artifact_repository import (
        CodeArtifactRepository,
    )
    from aichemist_archivum.domain.services.interfaces.code_analysis_service import (
    ...
    CodeArtifact = Any  # type: ignore
    CodeArtifactRepository = Any  # type: ignore
    CodeAnalysisServiceInterface = object  # type: ignore
...
class TechnicalCodeAnalyzer(CodeAnalysisServiceInterface):
    ...
    async def analyze_file(
        ...
        _content, analysis = await process_file(file_path)
    ...
    try:
        ...
        functions_out = [
            {
                "name": f["name"],
                "args": f["args"],
                "lineno": f.get("lineno", f.get("line", 0)),  # Support both keys
        ...
        for c in structure["classes"]:
            methods_out = [
                {
                    "name": m["name"],
                    "args": m["args"],
                    "lineno": m.get("lineno", m.get("line", 0)),  # Support both keys
            ...
            classes_out.append(
                {
                    "name": c["name"],
                    "lineno": c.get("lineno", c.get("line", 0)),  # Support both keys
