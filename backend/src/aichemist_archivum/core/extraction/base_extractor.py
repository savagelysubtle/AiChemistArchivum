...
class BaseMetadataExtractor(ABC):
    ...
    async def extract_with_content(
        self,
        file_path: str | Path,
        content: str | bytes,  # Changed to str | bytes to match ingestion_service usage
        # mime_type and metadata could be added if extractors need them here too
    ) -> T | None:
        """Extract metadata using pre-loaded content.
        Optional method for extractors to implement if they can benefit from it.
        Base implementation can return None or raise NotImplementedError
        if not overridden by subclasses that support this.
        """
        # Default implementation indicates it's not supported or needs override
        # Option 1: Do nothing / return None
        return None
        # Option 2: Raise NotImplementedError
        # raise NotImplementedError(
        #     f"{self.__class__.__name__} does not support extract_with_content directly. "
        #     f"Use extract() method or override extract_with_content."
        # )
