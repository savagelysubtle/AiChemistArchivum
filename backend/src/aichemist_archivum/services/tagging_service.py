...
from aichemist_archivum.core.tagging.schema import TagSchema
...
class TagManager:
    ...
    async def create_tag(self, name: str, description: str = "") -> int:
        ...
        try:
            ...
            result_raw = await self.schema.db.fetchone(
                "SELECT id FROM tags WHERE name = ?", (name,), return_format="dict"
            )
            result = cast(dict[str, Any] | None, result_raw)
            if result:
                tag_id = result["id"]
    ...
    async def get_tag(self, tag_id: int) -> dict[str, Any] | None:
        ...
        try:
            result_raw = await self.schema.db.fetchone(
                "SELECT * FROM tags WHERE id = ?", (tag_id,), return_format="dict"
            )
            result = cast(dict[str, Any] | None, result_raw)
            if result:
                return {
    ...
    async def get_all_tags(self) -> list[dict[str, Any]]:
        ...
        """
        try:
            results_raw = await self.schema.db.fetchall(
                "SELECT * FROM tags ORDER BY name", return_format="dict"
            )
    ...
    async def add_file_tag(
        ...
        try:
            ...
            raise RuntimeError("Internal error: tag_id is None after processing") from e
        except Exception as e:
            logger.error(
    ...
    async def remove_file_tag(self, file_path: Path, tag_id: int) -> bool:
        ...
        try:
            ...
            result_raw = await self.schema.db.fetchone(
                "SELECT 1 as exists_flag FROM file_tags WHERE file_path = ? AND tag_id = ?",
                (file_path_str, tag_id),
                return_format="dict",
            )
            result = cast(dict[str, Any] | None, result_raw)
            if not result or not result["exists_flag"]:
                logger.debug(
    ...
    async def remove_orphaned_tags(self) -> int:
        ...
        try:
            ...
            orphaned_tag_ids = [row["id"] for row in results]
    ...
    async def clean_missing_files(self) -> int:
        ...
        try:
            # Get all file paths
            results_raw = await self.schema.db.fetchall(
                "SELECT DISTINCT file_path FROM file_tags", return_format="dict"
            )
            results = cast(list[dict[str, Any]], results_raw)

            all_paths = [row["file_path"] for row in results]
            missing_paths = []
    ...
    async def get_tag_suggestions(self, file_path: Path) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if extension:
                ext_results_raw = await self.schema.db.fetchall(
                    """
                ...
                ext_results = cast(list[dict[str, Any]], ext_results_raw)

                # Add to suggestions with weight based on frequency
                max_count = (
                    max([row["count"] for row in ext_results]) if ext_results else 1
                )
                for row in ext_results:
                    _tag_id, tag_name, count = row["id"], row["name"], row["count"]
            ...
            dir_results_raw = await self.schema.db.fetchall(
                """
                SELECT t.id, t.name, COUNT(*) as count
                FROM file_tags ft
                JOIN tags t ON ft.tag_id = t.id
            ...
            dir_results = cast(list[dict[str, Any]], dir_results_raw)

            # Add to suggestions with weight based on frequency
            max_count = max([row["count"] for row in dir_results]) if dir_results else 1
