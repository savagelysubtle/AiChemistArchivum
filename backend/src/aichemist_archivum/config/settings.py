...
def get_data_dir() -> Path:
    ...
    if is_frozen():
        ...
        # Adapt determine_project_root logic for development as needed.
...
def get_config_dir() -> Path:
    ...
    if is_frozen():
        ...
        # Use config subdirectory in project root when running from source
        project_root = determine_project_root()  # Your existing function
        path = project_root / "config"
        logger.debug(f"Running from source, using project config dir: {path}")
...
def determine_project_root() -> Path:
    ...
    if "AICHEMIST_ROOT" in os.environ:
        ...
        if path.exists():
            ...
            logger.warning(
                f"AICHEMIST_ROOT set to '{os.environ['AICHEMIST_ROOT']}', "
    ...
    logger.warning(
        f"Could not reliably determine project root. "
        f"Using last resort: {last_resort_path}"
    )
